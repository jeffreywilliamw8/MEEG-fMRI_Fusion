...
print('>>> Decoding-Encoding Fusion (testing) <<<')
...
encoder_weights_file = f'/scratch/jeffreykatab/Code/Encoding_Models/THINGS/regression_weights/decoding_encoding/roi/{args.modality}/fmri_sub-{args.fmri_subject:02d}/encoder_weights_{args.roi}.npy'
encoder_weights = np.load(encoder_weights_file, allow_pickle=True).item()

decoder_weights_file = f'/scratch/jeffreykatab/Code/Encoding_Models/THINGS/regression_weights/decoding_encoding/roi/{args.modality}/fmri_sub-{args.fmri_subject:02d}/decoder_weights_{args.roi}.npy'
decoder_weights = np.load(decoder_weights_file, allow_pickle=True).item()
...
for t in tqdm(range(len(times))):
    ...
    decoder.coef_ = decoder_weights['coef_'][t]
    decoder.intercept_ = decoder_weights['intercept_'][t]
    decoded_features = decoder.predict(meg_test[:,:,t])

    

    # Instantiate the fusion regression model
    encoder = LinearRegression()
    encoder.coef_ = encoder_weights['coef_'][t]
    encoder.intercept_ = encoder_weights['intercept_'][t]
    tfmri = encoder.predict(decoded_features)
    ...
    del tfmri, tfmri_z, decoder, encoder
    gc.collect()
...
corrs_dir = f'/scratch/jeffreykatab/Code/Encoding_Models/THINGS/correlations/decoding_encoding/roi/{args.modality}/fmri_sub-{args.fmri_subject:02d}'
